            ...
            VStack(spacing: 16) {
                ...
                VStack(spacing: 8) {
                    Text(manga.title)
                        .font(.title2)
                        .fontWeight(.bold)
                        .foregroundColor(colorScheme == .dark ? .white : .black) // Adapta el texto
                        .multilineTextAlignment(.center)
                    
                    Text(manga.authors.joinedStringValues(separator: " • ", keyPath: \.fullName))
                        .font(.subheadline)
                        .foregroundColor((colorScheme == .dark ? Color.white : .black).opacity(0.8))
                        .multilineTextAlignment(.center)
...
// Status badge que respeta el color scheme
